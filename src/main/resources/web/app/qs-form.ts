...
export const QS_RESULT_NEXT_EVENT = 'qs-result-next';
...
export class QsForm extends LitElement {
  ...
  @property({type: String}) api: string = "/api/search";
  ...
  private _search = () => {
    ...
    this._jsonFetch(new AbortController(), 'GET', data, 1000)
      .then((r: any) => {
        ...
        if (this._page > 0 && r.hits.length > 0) {
          this._total += r.hits.length;
          event = QS_RESULT_NEXT_EVENT;
        } else {
          this._total = r.hits.length;
        }
        const hasMoreHits = r.total > this._total;

        this.dispatchEvent(new CustomEvent(event, {detail: {...r, hasMoreHits}}));
      }).finally(() => {
      this._loading = false;
